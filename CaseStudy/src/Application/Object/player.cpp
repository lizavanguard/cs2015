...
void Player::Update(Uriel *uriel_){
  ...
  if ((pJoypad.IsTrigger(InputDevice::Pads::PAD_A) || pKeyboard.IsTrigger(DIK_RETURN)) && !call_stop_flag_) {
...
const D3DXVECTOR3 Player::GetBoroPos(void) const {
  if (move_direction_ == DIRECTION_RIGHT){
    ...
    D3DXVECTOR3 pos = pos_ + kBoroPosOffset;
    pos.x = pos_.x - kBoroPosOffset.x;
    return pos;
  }
}
